...
// 添加全局变量或使用 chip->charge_full_design
static unsigned int user_charge_full = 5400000;  // 默认5400mAh

// 定义 sysfs 的 show 和 store 函数
static ssize_t charge_full_show(struct device *dev,
                struct device_attribute *attr, char *buf)
{
    struct qpnp_qg *chip = dev_get_drvdata(dev);
    return sprintf(buf, "%u\n", chip->charge_full_design);
}

static ssize_t charge_full_store(struct device *dev,
                ...
                const char *buf, size_t count)
{
    ...
    // 单位转换（假设用户输入的是mAh）
    chip->charge_full_design = value ;  // 转换为µAh
    return count;
}

// 注册 sysfs 属性
static DEVICE_ATTR_RW(charge_full);
...
{
    struct qpnp_qg *chip = dev_get_drvdata(dev);
    int value_mv, value_uv;

    // 用户输入的是毫伏（mV），需转为微伏（µV）
    if (kstrtoint(buf, 0, &value_mv))
        return -EINVAL;

    value_uv = value_mv * 1000;  // 1mV = 1000µV

    // 检查电压是否在安全范围内（例如：3700mV ~ 4540mV）
    if (value_uv < 3700000 || value_uv > 4540000) {
        pr_err("Invalid voltage_max: %d µV\n", value_uv);
        return -EINVAL;
    }

    // 更新电压值并通知充电子系统
    chip->voltage_max_design = value_uv;
    power_supply_changed(chip->batt_psy);  // 触发电源状态更新
...
static int qpnp_qg_probe(struct platform_device *pdev)
{
    ...
    if (!chip->regmap) {
        dev_err(&pdev->dev, "Parent regmap is unavailable\n");
        return -ENXIO;
    }
    ...
    rc = qg_parse_dt(chip);
    if (rc < 0) {
        dev_err(&pdev->dev, "Failed to parse DT rc=%d\n", rc);
        return rc;
    }

    /* 初始化电压默认值（单位：µV） */
    chip->voltage_max_design = 4400000;  // 默认4.4V
    if (of_property_read_u32(pdev->dev.of_node, "qcom,voltage-max-design",
                            &chip->voltage_max_design))
        dev_info(&pdev->dev, "Using default voltage_max: %d µV\n",
                chip->voltage_max_design);

    /* 注册 sysfs 属性 */
    rc = device_create_file(&pdev->dev, &dev_attr_voltage_max);
    if (rc) {
        dev_err(&pdev->dev, "Failed to create voltage_max sysfs: %d\n", rc);
        goto cleanup;
    }
    ...
    rc = qg_init(chip);
    if (rc < 0) {
        dev_err(&pdev->dev, "QG init failed: %d\n", rc);
        goto cleanup_sysfs;
    }
...
cleanup_sysfs:
    device_remove_file(&pdev->dev, &dev_attr_voltage_max);
cleanup:
